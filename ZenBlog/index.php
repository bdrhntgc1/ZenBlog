...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo '<div style="border-bottom: 1px solid #ccc; margin-bottom: 20px; padding-bottom: 10px;">';
        echo '<h2>' . htmlspecialchars($row['title']) . '</h2>';
        echo '<p><i>Yazar: ' . htmlspecialchars($row['author']) . ' | ' . $row['created_at'] . '</i></p>';
        echo '<p>' . nl2br(htmlspecialchars($row['content'])) . '</p>';
        echo '</div>';
    }
    echo '</div>';
} else {
    echo '<p style="text-align: center;">Henüz yazı eklenmemiş.</p>';
}
?>
